...
from models.blocks import PatchEmbed, PositionGetterQuant  # noqa
...
class PatchEmbedDust3RQuant(PatchEmbed):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.position_getter = PositionGetterQuant()

    def forward(self, x, **kw):
        x = self.proj(x)
        pos = self.position_getter(x)
        if self.flatten:
            x = x.flatten(2).transpose(1, 2)  # BCHW -> BNC
        x = self.norm(x)
        return x, pos
